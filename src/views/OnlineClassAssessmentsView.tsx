...
export const OnlineClassAssessmentsView: React.FC = () => {
  ...
  const filteredAssessments = liveAssessments.filter((ass) => {
    ...
    const matchesStatus =
      ...
      (selectedStatus === 'closed' && (ass.status === 'closed' || ass.status === 'published'));
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-3 pt-1">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              >
                ...
                <option value="closed">Past / Closed Assessments</option>
                ...
                <div className="p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  <div className="space-y-2 flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {ass.status === 'closed' && (
                        <span className="px-2.5 py-0.5 rounded-md text-[10px] font-extrabold uppercase bg-purple-100 text-purple-700 border border-purple-200 flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3 text-purple-600" />
                          Past Assessment / Closed
                        </span>
                      )}
                      {ass.isDraft && (
                    ...
                    <div className="flex items-center gap-4 text-xs text-slate-500 flex-wrap pt-1">
                      ...
                      <span className="flex items-center gap-1.5 font-bold text-slate-700 dark:text-slate-300">
                        ...
                        Average Score: {ass.submissions.length > 0 ? `${Math.round(ass.submissions.reduce((sum, s) => sum + s.percentage, 0) / ass.submissions.length)}%` : 'N/A'}
                      </span>
                      <span>•</span>
                      ...
                      <button
                        ...
                        className="text-[11px] font-bold text-[#c26d15] hover:text-orange-700 bg-[#fff4e6] hover:bg-orange-100 dark:bg-amber-950/40 px-2 py-0.5 rounded-md border border-[#fcd8b3] dark:border-amber-800/40 transition flex items-center gap-1 cursor-pointer"
                        title="Click to open submissions in Review Studio"
                      >
                        <Award className="w-3 h-3 text-[#f39223]" />
                        <span>Submissions: {ass.submissions.length}</span>
                      </button>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row lg:flex-col items-stretch lg:items-end justify-center gap-2 shrink-0">
                    <button
                      onClick={() => openAssessmentSubmissionsReview(ass)}
                      className="inline-flex items-center justify-center gap-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 text-xs font-extrabold shadow-sm hover:shadow-md transition-all cursor-pointer"
                      title="View all candidate submissions and detailed question answers in Review Studio"
                    >
                      <Award className="w-3.5 h-3.5 text-amber-300" />
                      <span>View Submissions ({ass.submissions.length})</span>
                    </button>
